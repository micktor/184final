...
with open('dataframe.csv','a') as dataframe:
    for i in range (0,int(number_of_pages)):
        ...
        for j in range (0,100):
            ...
            item_title = data['findCompletedItemsResponse'][0]['searchResult'][0]['item'][j]['title'][0]
            try:
            ...
            view_item = 'https://api.ebay.com/buy/browse/v1/item/v1|'+str(item_number)+'|0'
            try:
                r = requests.get(view_item, headers=headers)
                data2 = r.json()
                # print(data2)

                # short_description = data['shortDescription']
                seller = data2['seller']['username']
                feedback_percentage = data2['seller']['feedbackPercentage']
                feedback_score = data2['seller']['feedbackScore']
                description = str(data2['description'])
            ...
            line = [item_number, item_title, payment_method, postal_code, shipping_method, sales_price, selling_state, best_offer, bin_available, start_time, end_time, listing_type, returns_accepted, condition, top_rated, seller, feedback_percentage, feedback_score, description+'\n']
